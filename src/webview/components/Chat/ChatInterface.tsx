...
import CustomUrlConfig from './CustomUrlConfig';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ layout, vscode }) => {
    ...
    const [customBaseUrlEnabled, setCustomBaseUrlEnabled] = useState<boolean>(false); // 添加自定义URL状态
    ...
    useEffect(() => {
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if (message.command === 'currentProviderResponse') {
                ...
                // 检查是否启用了自定义URL
                if (message.useCustomBaseUrl !== undefined) {
                    setCustomBaseUrlEnabled(message.useCustomBaseUrl);
                }
            ...
            } else if (message.command === 'customBaseUrlChanged') {
                // 处理自定义URL状态变化
                setCustomBaseUrlEnabled(message.enabled);
            }
        };
                        ...
                        <div className="input-controls">
                            <div className="selectors-group">
                                <div className="selector-wrapper">
                                    ...
                                    <CustomUrlConfig 
                                        vscode={vscode}
                                        isEnabled={customBaseUrlEnabled}
                                        provider={selectedModel?.includes('/') ? 'openrouter' : 
