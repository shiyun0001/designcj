...
export class ChatSidebarProvider implements vscode.WebviewViewProvider {
    ...
    private async handleGetCurrentProvider(webview: vscode.Webview) {
        ...
        const useCustomBaseUrl = config.get<boolean>('useCustomBaseUrl', false); // 获取自定义URL状态
        ...
        webview.postMessage({
            ...
            useCustomBaseUrl: useCustomBaseUrl // 传递自定义URL状态
        });
    }
        ...
        try {
            ...
            if (!apiKey) {
                const result = await vscode.window.showWarningMessage(
                    `已选择 ${displayName}，但未配置API密钥。现在要配置API密钥吗？`,
                    '配置API密钥',
                    '稍后'
                );
            ...
            vscode.window.showErrorMessage(`更新AI模型失败: ${error}`);
        }
    }
