...
export class CustomAgentService implements AgentService {
    ...
    private getModel() {
        ...
        this.outputChannel.appendLine(`使用 AI 提供商: ${provider}`);
        if (specificModel) {
            this.outputChannel.appendLine(`使用特定 AI 模型: ${specificModel}`);
        }
        ...
        switch (effectiveProvider) {
            case 'openrouter':
                ...
                if (!openrouterKey) {
                    throw new Error('未配置 OpenRouter API 密钥。请运行"配置 OpenRouter API 密钥"命令。');
                }
                
                this.outputChannel.appendLine(`OpenRouter API 密钥已找到: ${openrouterKey.substring(0, 12)}...`);
                
                const openrouterOptions: any = {
                    apiKey: openrouterKey
                };

                // 如果启用了自定义URL，则添加baseURL
                if (useCustomBaseUrl) {
                    const customBaseUrl = config.get<string>('openrouterBaseUrl', 'https://openrouter.ai/api/v1');
                    this.outputChannel.appendLine(`使用自定义 OpenRouter API URL: ${customBaseUrl}`);
                    openrouterOptions.baseURL = customBaseUrl;
                }

                const openrouter = createOpenRouter(openrouterOptions);
                ...
                this.outputChannel.appendLine(`使用 OpenRouter 模型: ${openrouterModel}`);
                return openrouter.chat(openrouterModel);
                
            case 'anthropic':
                const anthropicKey = config.get<string>('anthropicApiKey');
                if (!anthropicKey) {
                    throw new Error('未配置 Anthropic API 密钥。请运行"配置 Anthropic API 密钥"命令。');
                }
                
                this.outputChannel.appendLine(`Anthropic API 密钥已找到: ${anthropicKey.substring(0, 12)}...`);
                
                const anthropicOptions: any = {
                    apiKey: anthropicKey
                };

                // 如果启用了自定义URL，则使用自定义URL，否则使用Helicone代理
                if (useCustomBaseUrl) {
                    const customBaseUrl = config.get<string>('anthropicBaseUrl', 'https://api.anthropic.com/v1');
                    this.outputChannel.appendLine(`使用自定义 Anthropic API URL: ${customBaseUrl}`);
                    anthropicOptions.baseURL = customBaseUrl;
                } else {
                    anthropicOptions.baseURL = "https://anthropic.helicone.ai/v1";
                ...
                this.outputChannel.appendLine(`使用 Anthropic 模型: ${anthropicModel}`);
            ...
            default:
                ...
                if (!openaiKey) {
                    throw new Error('未配置 OpenAI API 密钥。请运行"配置 OpenAI API 密钥"命令。');
                }
                
                this.outputChannel.appendLine(`OpenAI API 密钥已找到: ${openaiKey.substring(0, 7)}...`);
                ...
                // 如果启用了自定义URL，则使用自定义URL，否则使用Helicone代理
                if (useCustomBaseUrl) {
                    const customBaseUrl = config.get<string>('openaiBaseUrl', 'https://api.openai.com/v1');
                    this.outputChannel.appendLine(`使用自定义 OpenAI API URL: ${customBaseUrl}`);
                    openaiOptions.baseURL = customBaseUrl;
                } else {
                    openaiOptions.baseURL = "https://oai.helicone.ai/v1";
                ...
                this.outputChannel.appendLine(`使用 OpenAI 模型: ${openaiModel}`);
