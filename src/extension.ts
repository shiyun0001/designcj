...
async function initializeSuperdesignProject() {
	const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
	if (!workspaceFolder) {
		vscode.window.showErrorMessage('未找到工作区文件夹。请先打开一个工作区。');
		return;
	}
	...
	// 确保配置了默认值
	if (!config.has('useCustomBaseUrl')) {
		await config.update('useCustomBaseUrl', false, vscode.ConfigurationTarget.Global);
	}
	if (!config.has('anthropicBaseUrl')) {
		await config.update('anthropicBaseUrl', 'https://api.anthropic.com/v1', vscode.ConfigurationTarget.Global);
	}
	if (!config.has('openaiBaseUrl')) {
		await config.update('openaiBaseUrl', 'https://api.openai.com/v1', vscode.ConfigurationTarget.Global);
	}
	if (!config.has('openrouterBaseUrl')) {
		await config.update('openrouterBaseUrl', 'https://openrouter.ai/api/v1', vscode.ConfigurationTarget.Global);
	}
...
export function activate(context: vscode.ExtensionContext) {
	...
	// 注册配置自定义 API 基础 URL 的命令
	const configureCustomBaseUrlDisposable = vscode.commands.registerCommand('superdesign.configureCustomBaseUrl', async () => {
		await configureCustomBaseUrl();
	});
	...
	const resetWelcomeDisposable = vscode.commands.registerCommand('superdesign.resetWelcome', () => {
		...
		vscode.window.showInformationMessage('欢迎页面已重置。刷新侧边栏以查看欢迎页面。');
	});
	...
	context.subscriptions.push(
		...
		configureCustomBaseUrlDisposable,
		sidebarDisposable,
...
async function configureAnthropicApiKey() {
	...
	const input = await vscode.window.showInputBox({
		title: '配置 Anthropic API 密钥',
		prompt: '请输入您的 Anthropic API 密钥 (从 https://console.anthropic.com/ 获取)',
		value: currentKey ? '••••••••••••••••' : '',
		password: true,
		placeHolder: 'sk-ant-...',
		validateInput: (value) => {
			...
			if (!value.startsWith('sk-ant-')) {
				return 'Anthropic API 密钥应以 "sk-ant-" 开头';
			}
			return null;
		}
	});
	...
	if (input !== undefined) {
		...
		if (input !== '••••••••••••••••') {
			try {
				...
				vscode.window.showInformationMessage('✅ Anthropic API 密钥配置成功！');
			} catch (error) {
				vscode.window.showErrorMessage(`保存 API 密钥失败：${error}`);
			}
		} else if (currentKey) {
			vscode.window.showInformationMessage('API 密钥未更改 (已配置)');
		} else {
			vscode.window.showWarningMessage('未设置 API 密钥');
		}
	}
}
...
async function configureOpenAIApiKey() {
	...
	const input = await vscode.window.showInputBox({
		title: '配置 OpenAI API 密钥',
		prompt: '请输入您的 OpenAI API 密钥 (从 https://platform.openai.com/api-keys 获取)',
		value: currentKey ? '••••••••••••••••' : '',
		password: true,
		placeHolder: 'sk-...',
		validateInput: (value) => {
	...
	if (input !== undefined) {
		...
		if (input !== '••••••••••••••••') {
			try {
				...
				vscode.window.showInformationMessage('✅ OpenAI API 密钥配置成功！');
			} catch (error) {
				vscode.window.showErrorMessage(`保存 API 密钥失败：${error}`);
			}
		} else if (currentKey) {
...
async function configureOpenRouterApiKey() {
	...
	const input = await vscode.window.showInputBox({
		title: '配置 OpenRouter API 密钥',
		prompt: '请输入您的 OpenRouter API 密钥 (从 https://openrouter.ai/ 获取)',
		value: currentKey ? '••••••••••••••••' : '',
		password: true,
		placeHolder: 'sk-...',
		validateInput: (value) => {
			...
			if (!value.startsWith('sk-')) {
				return 'OpenRouter API 密钥应以 "sk-" 开头';
			}
			return null;
		}
	});
	...
	if (input !== undefined) {
		...
		if (input !== '••••••••••••••••') {
			try {
				...
				vscode.window.showInformationMessage('✅ OpenRouter API 密钥配置成功！');
			} catch (error) {
				vscode.window.showErrorMessage(`保存 API 密钥失败：${error}`);
			}
		} else if (currentKey) {
...
// Function to configure custom API base URLs
async function configureCustomBaseUrl() {
	// 获取当前设置
	const config = vscode.workspace.getConfiguration('superdesign');
	const useCustomBaseUrl = config.get<boolean>('useCustomBaseUrl', false);
	const anthropicBaseUrl = config.get<string>('anthropicBaseUrl', 'https://api.anthropic.com/v1');
	...
	// 首先选择是否启用自定义URL
	const enableOption = await vscode.window.showQuickPick(
		[
			{ label: '是', description: '启用自定义API基础URL', value: true },
			{ label: '否', description: '使用默认API基础URL', value: false },
		],
		{
			placeHolder: '是否启用自定义API基础URL?',
			title: '配置自定义API基础URL'
		}
	);
	...
	// 更新启用状态
	await config.update('useCustomBaseUrl', enableOption.value, vscode.ConfigurationTarget.Global);

	if (enableOption.value) {
		// 如果启用，配置各API提供商的自定义URL
		
		// 选择要配置的API提供商
		const providerOption = await vscode.window.showQuickPick(
			[
				{ label: 'Anthropic', description: '配置 Anthropic API 基础URL', value: 'anthropic' },
				{ label: 'OpenAI', description: '配置 OpenAI API 基础URL', value: 'openai' },
				{ label: 'OpenRouter', description: '配置 OpenRouter API 基础URL', value: 'openrouter' }
			],
			{
				placeHolder: '选择要配置的API提供商',
				title: '配置自定义API基础URL'
			}
		);
		...
		// 根据选择的提供商配置URL
		switch (providerOption.value) {
			case 'anthropic':
				const newAnthropicUrl = await vscode.window.showInputBox({
					title: '配置 Anthropic API 基础URL',
					prompt: '请输入 Anthropic API 的基础URL',
				...
				if (newAnthropicUrl) {
					await config.update('anthropicBaseUrl', newAnthropicUrl.trim(), vscode.ConfigurationTarget.Global);
					vscode.window.showInformationMessage(`✅ Anthropic API 基础URL已更新为: ${newAnthropicUrl}`);
				}
				break;

			case 'openai':
				const newOpenAIUrl = await vscode.window.showInputBox({
					title: '配置 OpenAI API 基础URL',
					prompt: '请输入 OpenAI API 的基础URL',
					value: openaiBaseUrl,
				...
				if (newOpenAIUrl) {
					await config.update('openaiBaseUrl', newOpenAIUrl.trim(), vscode.ConfigurationTarget.Global);
					vscode.window.showInformationMessage(`✅ OpenAI API 基础URL已更新为: ${newOpenAIUrl}`);
				}
				break;

			case 'openrouter':
				const newOpenRouterUrl = await vscode.window.showInputBox({
					title: '配置 OpenRouter API 基础URL',
					prompt: '请输入 OpenRouter API 的基础URL',
				...
				if (newOpenRouterUrl) {
					await config.update('openrouterBaseUrl', newOpenRouterUrl.trim(), vscode.ConfigurationTarget.Global);
					vscode.window.showInformationMessage(`✅ OpenRouter API 基础URL已更新为: ${newOpenRouterUrl}`);
				}
				break;
		}
	} else {
		vscode.window.showInformationMessage('已禁用自定义API基础URL，将使用默认URL');
	}
}
